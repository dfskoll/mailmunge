...
# Command to run mm-mx-ctrl
set MM_MX_CTRL "mm-mx-ctrl -i -b"
...
if {[info exists env(MM_MX_CTRL)]} {
    set x $env(MM_MX_CTRL)
    if {"$x" != ""} {
	set MM_MX_CTRL $x
    }
}
...
proc open_connection { mach } {
    ...
    set fp [open "| $SSHCommand $hmach $MM_MX_CTRL" "r+"]
...
proc log_stats { mach line } {
    set dir "~/.watch-multiple-mailmunges/$mach"
...
toplevel .top
wm title .top "Watch Multiple Mailmunges"
wm iconname .top "Mailmunges"
wm withdraw .top
reconfigure
wm deiconify .top
update

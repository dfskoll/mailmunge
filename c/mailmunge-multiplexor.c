...
#define STR(x) STR2(x)
#define STR2(x) #x
#define MAX_CMD_LEN 4096	/* Maximum length of command from mailmunge */
...
struct Settings_t {
    ...
    char const *sockName;	/* Socket name for talking to mailmunge    */
...
/**********************************************************************
* %FUNCTION: reply_to_mailmunge_with_len
* %ARGUMENTS:
*  es -- event selector
*  fd -- file descriptor
...
* %DESCRIPTION:
*  Sends a final message back to mailmunge.  Closes fd after message has
*  been sent.
***********************************************************************/
static EventTcpState *
reply_to_mailmunge_with_len(EventSelector *es,
			     int fd,
			     char const *msg,
			     int len)
{
    ...
    if (!e) {
	if (DOLOG) {
	    syslog(LOG_ERR, "reply_to_mailmunge: EventTcp_WriteBuf failed: %m");
	}
	close(fd);
    }
    return e;
...
*  The event associated with the reply, or NULL.
* %DESCRIPTION:
*  Sends a final message back to mailmunge.
***********************************************************************/
EventTcpState *
reply_to_mailmunge(EventSelector *es,
		    int fd,
		    char const *msg)
{
    return reply_to_mailmunge_with_len(es, fd, msg, strlen(msg));
}
...
usage(void)
{
    fprintf(stderr, "mailmunge-multiplexor version %s\n", VERSION);
    fprintf(stderr, "Usage: mailmunge-multiplexor [options]\n");
    ...
    fprintf(stderr, "  -d                -- Debug events in /var/log/mailmunge-event-debug.log\n");
...
extern char **save_ps_display_args(int, char **);
...
main(int argc, char *argv[], char **env)
{
    ...
    Settings.progPath      = CONFDIR "/mailmunge-filter.pl";
    ...
    options = "GAa:Tt:um:x:y:r:i:b:c:s:hdlf:p:o:w:W:U:S:q:Q:I:DEO:X:Y:N:vZP:z:V:";
#else
    options = "GAa:Tt:um:x:y:r:i:b:c:s:hdlf:p:o:w:W:U:S:q:Q:L:R:M:I:DEO:X:Y:N:vZP:z:V:";
#endif
	...
	case 'a':
	    ...
	    break;
	case 'v':
	    printf("mailmunge-multiplexor version %s\n", VERSION);
	    exit(EXIT_SUCCESS);
...
#else
	    fprintf(stderr, "mailmunge-multiplexor compiled without support for embedded perl.  Ignoring -E flag.\n");
#endif
	    break;
	case 'D':
	    nodaemon = 1;
    ...
    if (!Settings.sockName) {
	Settings.sockName = malloc(strlen(Settings.spoolDir) + strlen("/mailmunge-multiplexor.sock") + 1);
	    if (!Settings.sockName) {
		fprintf(stderr, "%s: Out of memory\n", argv[0]);
		exit(EXIT_FAILURE);
	    }
	    strcpy((char *) Settings.sockName, Settings.spoolDir);
	    strcat((char *) Settings.sockName, "/mailmunge-multiplexor.sock");
    }
    ...
    if (!getuid() || !geteuid()) {
	fprintf(stderr,
		"ERROR: You must not run mailmunge-multiplexor as root.\n"
	...
	openlog("mailmunge-multiplexor", LOG_PID|LOG_NDELAY, facility);
    }
...
*  Nothing
* %DESCRIPTION:
*  Handles a connection attempt from mailmunge.  Sets up a command-reader.
***********************************************************************/
static void
...
* %ARGUMENTS:
*  es -- event selector
*  fd -- connection to mailmunge
*  buf -- buffer of data read from mailmunge
*  len -- amount of data read from mailmunge
	      ...
	      void *data)
{
	...
	snprintf(answer, sizeof(answer), "%d\n",
		 NUM_FREE_WORKERS);
	reply_to_mailmunge(es, fd, answer);
	return;
    }

    if (len == 7 && !strcmp(buf, "version")) {
	reply_to_mailmunge(es, fd, VERSION);
	return;
    }
	...
	if (sscanf(buf+6, "%d", &back) != 1 ||
	    back < 10 || back > 600) {
	    reply_to_mailmunge(es, fd, "error: Invalid 'back' amount (must be 10-600)\n");
	    return;
	}
	doLoad1(es, fd, back);
	return;
    ...
    /* This is an awful hack used by watch-multiple-mailmunges.tcl.
       We handle it here so we don't have to waste a worker */
    if (len == 19 && !strcmp(buf, "foo_no_such_command")) {
	reply_to_mailmunge(es, fd, "error: Unknown command\n");
	return;
    }

    /* Remaining commands are privileged */
    if (data) {
	reply_to_mailmunge(es, fd, "error: Attempt to use privileged command on unprivileged socket\n");
	return;
    }
	...
	if (Settings.useEmbeddedPerl) {
	    reply_to_mailmunge(es, fd, "Cannot destroy and recreate an embedded Perl interpreter safely on this platform.  Filter rules will NOT be reread\n");
	    return;
	}
#endif
	reply_to_mailmunge(es, fd, "Forced reread of filter rules\n");
	return;
    }
...
doWorkerInfo(EventSelector *es, int fd, char *cmd)
{
	...
	reply_to_mailmunge(es, fd, "error: Invalid worker number\n");
	return;
    }
    if (workerno < 0 || workerno >= Settings.maxWorkers) {
	reply_to_mailmunge(es, fd, "error: Worker number out of range\n");
	return;
    }
...
doWorkerCommandAux(EventSelector *es, int fd, char *cmd, int queueable)
{
	...
	if (at_recipok_limit(cmd)) {
	    ...
	    reply_to_mailmunge(es, fd, "ok -1 Per-domain%20recipok%20limit%20hit;%20please%20try%20again%20later\n");
	    return;
	}
    }
			   ...
			   void *data)
{
	...
	reply_to_mailmunge(es, s->clientFD, answer);
	/* The reply_to_mailmunge will close clientFD when it's done */
	s->clientFD = -1;
			  ...
			  void *data)
{
	...
	if (flag == EVENT_TCP_FLAG_TIMEOUT) {
		...
		reply_to_mailmunge(es, s->clientFD,
		...
		if (s->oom) {
		    ...
		    syslog(LOG_ERR, "Worker %d died prematurely -- check your filter rules and use the '-l' flag on mailmunge-multiplexor to see Perl error messages", WORKERNO(s));
		}
	    }
	    reply_to_mailmunge(es, s->clientFD, "ERR No response from worker\n");
	    /* The reply_to_mailmunge will close clientFD when it's done */
	    s->clientFD = -1;
	}
    } else {
	/* Write the worker's answer back to the client */
	reply_to_mailmunge_with_len(es, s->clientFD, buf, len);
	/* The reply_to_mailmunge will close clientFD when it's done */
	s->clientFD = -1;
    }
...
activateWorker(Worker *s, char const *reason)
{
    ...
    /* Change to spool directory, ignoring errors, I guess */
    chdir(Settings.spoolDir);
...
doWorkerReport(EventSelector *es, int fd, int only_busy)
{
    ...
    int secs;
    if (!ans) {
	reply_to_mailmunge(es, fd, "error: Out of memory\n");
	return;
    }
    *ans = 0;
    now = time(NULL);
...
doHistogram(EventSelector *es, int fd)
{
  ...
  for (i=0; i<Settings.maxWorkers; i++) {
    ...
    if (count < 0 || count >= roomleft) {
      free(ans);
      reply_to_mailmunge(es, fd, "error: String too long\n");
      return;
    }
    pos += count;
    roomleft -= count;
			  ...
			  void *data)
{
    ...
    reply_to_mailmunge(es, fd, "error: Queued request timed out\n");
}

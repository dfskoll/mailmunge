...
extern EventTcpState *reply_to_mailmunge(EventSelector *es,
					  int fd,
					  char const *msg);
...
handle_notifier_accept(EventSelector *es, int fd) {
    ...
    if (!l) {
	reply_to_mailmunge(es, fd, "*ERR No free listening slots\n");
	return;
    }
    l->fd = fd;
...
setup_read_event(Listener *l, EventSelector *es)
{
    ...
    if (!l->read_ev) {
	reply_to_mailmunge(es, l->fd, "*ERR Unable to make reader event\n");
	l->fd = -1;
	close_listener(l);
    }
    return l->read_ev;

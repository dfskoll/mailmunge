...
#define SCAN_BODY "mailmunge " VERSION
...
envfrom(SMFICTX *ctx, char **from)
{
	...
	snprintf(buffer, SMALLBUF, "%s/mm-%s", SpoolDir, mxid);
    } else {
	/* We have a queue ID, so use it */
	snprintf(buffer, SMALLBUF, "%s/mm-%s", SpoolDir, data->qid);
    }
...
usage(void)
{
    fprintf(stderr, "mailmunge version %s\n", VERSION);
    fprintf(stderr, "Usage: mailmunge [options]\n");
...
main(int argc, char **argv)
{
    ...
    if (!getuid() || !geteuid()) {
	fprintf(stderr,
		"ERROR: You must not run mailmunge as root.\n"

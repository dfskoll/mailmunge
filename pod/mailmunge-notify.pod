...
mailmunge-notify - Conventions used by C<mailmunge-multiplexor> to
notify an external program of state changes.

=head1 DESCRIPTION

If you supply the C<-O> option to L<mailmunge-multiplexor>, then
...
The listener connects to a TCP or UNIX-domain socket.

=item 2

The listener informs C<mailmunge-multiplexor> of the I<message types>
it is interested in.

=item 3

The listener loops, reading messages from the socket and reacting to
them.
...
The parameter I<n> is the number of free workers.
...
This message is issued whenever worker I<n>'s status tag
changes.  The status tag is a string indicating what the worker
is currently doing; the C<-Z> option to the multiplexor
...
=head1 EXPRESSING INTEREST

A listener does not receive any messages until it has I<expressed interest>
...
Note that a listener I<always> receives the special messages
...
This script assumes that you have used the C<-O inet:4567> option
to C<mailmunge-multiplexor>.
...
L<mailmunge-multiplexor>, L<mailmunge>, L<mm-mx-ctrl>

...
The protocol is a simple line-based protocol.  The filter reads a
single line from standard input; this line contains a command.  The
filter executes the command and then outputs a single-line response
to standard-output.

For the C<scan> command, additional information is places in a
...
The filter program may be invoked in one of four ways.  It is
expected to run in a loop, accepting commands from standard
input and writing responses to standard output.  It should
exit if it receives end-of-file on standard input.
...
The server must write "ok 1" to standard output if we will accept the
connection, or "ok 0 error_message code dsn" if not.  It can indicate
a temporary failure by writing "ok -1 error_message code dsn".  Note
that even if the connection is accepted, a later scan can still reject
the message based on other criteria.  "ip_addr" is the IP address of
the relay and "hostname" is the hostname (if it could be determined;
otherwise, the IP address in square brackets).
...
write "ok 1" to standard output if we will accept the mail attempt, or
"ok 0 error_message code dsn" if not.  "ok -1 error_message code dsn"
indicates a temporary failure.  I<helo_string> is the argument to the
...
server must write "ok 1" to standard output if we will accept the mail
attempt, or "ok 0 error_message code dsn" if not.  "ok -1
error_message code dsn" indicates a temporary failure.  Note that even
if the sender is accepted, a later scan can still reject the message
based on other criteria.  "sender_addr" is the sender's e-mail
address. The "ip_addr" and "hostname" arguments are as in
C<relayok>. "helo_string" is the argument to the SMTP HELO/EHLO
command. "dir" is the Mailmunge spool directory, and "queue_id" is the
...
server must write "ok 1" to standard output if we will accept it, or
"ok 0 error_message code dsn" if not.  ok -1 error_message code dsn"
...
Server replies should be output on a single line on standard output.  The possible
replies are:

=over

=item ok [I<return_code>] [I<parameters>]
...
=item error: I<Message>

The operation failed.  I<Message> may give additional details.

=back

In server mode, you should I<not> write anything to standard output
except reply codes, or the multiplexor will become confused.  You
should I<not> terminate the program in server mode; simply echo an
...
terminate.  This is used by C<mailmunge-multiplexor> to terminate
...
(percent), "\" (backslash), "'" (single-quote) and double-quote, are
replaced by a percent sign followed by two hex digits specifying the
character's numerical value.  The filter must un-escape the arguments
when it reads the COMMANDS file.

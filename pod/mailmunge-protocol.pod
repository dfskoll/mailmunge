...
runs in a loop.  This is referred to as running in I<server> mode.
...
except reply codes as documented above, or the multiplexor will become
confused.  You should I<not> terminate the program in server mode;
simply echo an C<error:> reply and return to the server loop.
